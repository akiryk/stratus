...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<header class="entry-header">
		...
		<h1 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'stratus' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
		<div class="entry-meta">
			<span class="">Created on </span><?php echo esc_html( get_post_meta( get_the_ID(), 'date_project_completed', true ) ); ?></div>
	...
	<footer class="entry-meta">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
			<?php
				...
				$categories_list = get_the_category_list( __( ', ', 'stratus' ) );
				if ( $categories_list && stratus_categorized_blog() ) :
			?>
			<span class="cat-links">
				<?php printf( __( 'Posted in %1$s', 'stratus' ), $categories_list ); ?>
			</span>
